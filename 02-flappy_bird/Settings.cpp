...
void Settings::load_sounds()
{
    ...
    if (!buffer.loadFromFile(Settings::SOUNDS_PATH + "wood.wav"))
    {
        throw std::runtime_error{"Error loading sound sounds/wood.wav"};
    }

    result = Settings::sound_buffers.emplace("wood", buffer);

    sound.setBuffer(result.first->second);
    Settings::sounds["wood"] = sound;
