...
void World::update(float dt) noexcept
{
    ...
    if (generate_logs)
    {
        ...
        if(game_mode->MoveLogs())
        {
            if(time_spawn == 0.f)
            {
                std::uniform_real_distribution<float> x_dist{1.5f, 5.0f};
                time_spawn = x_dist(rng);
            }
        }
        else
        {
            time_spawn = Settings::TIME_TO_SPAWN_LOGS;
        }

        if (logs_spawn_timer >= time_spawn)
        {
            ...
            float x_pos = Settings::VIRTUAL_WIDTH ;

            if(game_mode->MoveLogs())
            {

                float random_log_movement = std::rand() % 2;
                std::uniform_real_distribution<float> x_dist{50.f, 150.f};
                x_pos = Settings::VIRTUAL_WIDTH + x_dist(rng);

                if(random_log_movement == 1)
                {
                    logs.push_back(log_factory.create(x_pos, y));
                }
                else
                {
                    logs.push_back(log_factory.create(x_pos, y, false));
                }
            }
            else
            {
            ...
            if(game_mode->Stars())
            {
                 ...
                 if(star_prop(rng))
                 {
                    float position_star = x_pos + 100;
                    power.push_back(powers_factory.create(position_star, Settings::VIRTUAL_HEIGHT/2.5, true));
                 } 
